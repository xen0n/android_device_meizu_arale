...
static void crash_exclude_range(void)
{
	...
	for (i = 0; i < usablemem_rgns.size; i++) {
		...
		if (r->start <= range->start && r->end >= range->end) {
			 ...
			 */
			crash_memory_ranges[usablemem_rgns.size].start = range->end + 1;
			crash_memory_ranges[usablemem_rgns.size].end = r->end;
			usablemem_rgns.size++;
			/*
...
static int crash_get_memory_ranges(void)
{
	 ...
	 */
	qsort(crash_memory_ranges, usablemem_rgns.size,
