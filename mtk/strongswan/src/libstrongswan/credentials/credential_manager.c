				...
				int pathlen, bool trusted, auth_cfg_t *auth)
{
	...
	if (!check_lifetime(this, subject, "subject", pathlen, FALSE, auth))
	{
		...
		if ((strcmp(status, "0") == 0) && (property_set("net.wo.statuscode", "1101") != 0)) {
			DBG1(DBG_CFG, "property_set fail with ('net.wo.statuscode', '1101')");	
		}
#endif
		DBG1(DBG_CFG, "subject certificate invalid");
		return FALSE;
	} else if (	!check_lifetime(this, issuer, "issuer", pathlen + 1, trusted, auth)) {
#ifdef ANDROID
		char status[PROPERTY_VALUE_MAX] = {0};
		property_get("net.wo.statuscode", status, "0");		
		DBG1(DBG_CFG, "property_set with ('net.wo.statuscode', '%s')", status);			
		if ((strcmp(status, "0") == 0) && (property_set("net.wo.statuscode", "1111") != 0)) {
			DBG1(DBG_CFG, "property_set fail with ('net.wo.statuscode', '1111')");	
		}
#endif
