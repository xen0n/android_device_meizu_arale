    ...
    while ((next = readdir(dir)) != NULL) {
        ...
        if (strcmp(name, pidName) == 0) {
            *number=strtol(next->d_name, NULL, 0);
            LOGD("findPidByName: rild pid is %d", *number);
            break;
        }
    }
    closedir(dir);
...
void ril_event_loop()
{
    ...
    for (;;) {
        ...
        if (n < 0) {
            ...
            if (errno == EBADF) {
                i++;
                LOGE("ril_event: select error EBADF");
                handle_bad_fdCTClientCommand();
                if(i == 20) {                                                                 //restart rild if  socket bad file number
                    LOGE("critical error!!   kill rild to restart");
                    killRild();
                }
                continue;
            }
