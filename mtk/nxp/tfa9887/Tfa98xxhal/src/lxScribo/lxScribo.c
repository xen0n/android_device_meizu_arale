...
{
    ALOGD("lxScriboRegister dev = %s", dev);
    if (lxScribo_verbose)
    {
    ...
    {
        ALOGD(" lxScriboInit = lxScriboSocketInit;");
        lxScriboInit = lxScriboSocketInit;
    }
    else if (strncmp(dev, "/dev/nxpspk",  11) == 0)
    {
        // if /dev/i2c... direct i2c device
        lxWrite = lxI2cWrite;
