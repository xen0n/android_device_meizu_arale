...
#endif 
//	VERBOSE ALOGD("%s is disabled, to be removed\n", __FUNCTION__);
	VERBOSE ALOGD("%s\n", __FUNCTION__);
...
NXP_I2C_Error_t i2cExecute(int num_write_bytes, unsigned char * data)
{
	...
	VERBOSE ALOGD("%s   W %3d: ",__FUNCTION__, num_write_bytes);
		...
		const unsigned char * WriteData, int NrOfReadBytes, unsigned char * ReadData)
{
	...
	VERBOSE ALOGD("%s W %d bytes:\t",__FUNCTION__, NrOfWriteBytes);
	...
	if (NrOfReadBytes ) {
		VERBOSE ALOGD("%s R %3d: ",__FUNCTION__, NrOfReadBytes);
...
//  error=eNone;//TODO handle errors properly at lower i2c levels
  ...
  switch (error)
  {
    ...
    default:
      /* unexpected error */
	  ALOGD("Got unexpected error 0x%08x\n", error);
      assert(0);
 ...
 TODO fix buffer
		...
		if (num_write_bytes > NXP_I2C_MAX_BURST) {
			...
			if (data[1] == 0x71 /*TFA98XX_CF_MAD*/) {
				...
				ALOGD(" chunks=%d, chunk_payload=%d, tail_payload=%d\n",
						chunks, chunk_payload, tail_payload);
				ALOGD("payload72=%d of %d\n", num_write_bytes - 2,
						num_write_bytes);
				chunkbuf[1] = 0x72;
